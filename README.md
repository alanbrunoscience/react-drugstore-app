...
O Projeto Drugstore App é um **Frontend** desenvolvido com o **Vite** e o **React** para consumir a API de uma Loja de Produtos Farmacéuticos (E-commerce), desenvolvida em **NestJS**. A aplicação permite o gerenciamento dos **Usuários**, **Produtos** e **Categorias**, além de utilizar autenticação por usuário e senha, com validação de token **JWT** para proteger as rotas e garantir a segurança da aplicação.
...
```bash
git clone git@github.com:alanbrunoscience/react-drugstore-app.git
cd react-drugstore-app
```
